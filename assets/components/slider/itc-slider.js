...
this.#config = {
  ...
  #init() {
    ...
    if (this.#config.effect === 'fade') {
      this.#state.el.classList.add('itc-slider-effect-fade');
    } else {
      this.#state.el.classList.remove('itc-slider-effect-fade');
    }
  ...
  #moveFade() {
    const items = this.#state.elListItem;
    const activeIndex = this.#state.activeItems.findIndex(i => i === 1);
    ...
    this.#state.activeItems = this.#state.activeItems.map((_, i) =>
      i === newIndex ? 1 : 0
    );
